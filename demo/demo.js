...
// Read carousel dimensions from CSS (responsive)
function getCarouselDimensions() {
  const style = getComputedStyle(document.documentElement);
  const step =
    parseFloat(style.getPropertyValue("--carousel-step").trim()) || 110;
  const offset =
    parseFloat(style.getPropertyValue("--carousel-left-offset").trim()) || 25;
  return { step, offset };
}

// Calculate left position for a given index
function getLeftPosition(index) {
  const { step, offset } = getCarouselDimensions();
...
function updateDisplay(animate = false, direction = null) {
  ...
  const { step } = getCarouselDimensions();
  ...
  currentQueue.forEach((item, currentIdx) => {
    ...
    if (itemElements.has(item)) {
      ...
      if (animate && direction) {
        ...
        if (direction === "next") {
          div.style.left = getLeftPosition(4) + step + "px";
        } else {
          div.style.left = getLeftPosition(0) - step + "px";
        }
      } else {
        div.style.left = targetLeft + "px";
      }
...
// Reapply positions on resize (e.g. orientation change or responsive breakpoint)
let resizeTimeout;
